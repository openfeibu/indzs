...
$(function(){
	...
	$('.hanle_count').click(function(){
		...
		if($(this).hasClass('fa-plus-circle'))
		{
			// if(count <=4)
			// {
				var k = $('.major_score_group').length + 1;
				var html = '<div class="form-group major_score_group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 考试科目'+k+'： </label><div class="col-sm-10"><input type="text" name="subject[]" value="" /></div></div>';
	...
	$('.hanle_room_subject_count').click(function(){
		var count = $(".subject_more").length;
		if($(this).hasClass('fa-plus-circle'))
		{
			var options = $(this).prev('select').html();
			var select_option_index =  $(this).prev('select').find("option:selected").index();
			var html = '<select name="subject[]"  class="col-sm-5 major_more" style="width:20%;margin-right:10px;" required>' + options + '</select>';
			$(this).before(html);
