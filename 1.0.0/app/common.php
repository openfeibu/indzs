...
function get_news($tag,$ispage=false,$pagesize=10,$type=null,$v=null,$where=array(),$currentPage=null)
{
    ...
    if($ispage){
        ...
        //不使用分页
        $news=Db::name("news")->alias("a")->field($field)->where($where_str)->where($where)->order($order)->limit($limit)->select();
        return $news;
    }
}
/**
...
function handle_score_arr($score_key,$score_arr)
{
	$arr = array();
	foreach ($score_key as $key => $value) {
		$arr[$value] = isset($score_arr[$key]) ? $score_arr[$key] : '';
	}
	return $arr;
}
function handle_score($arr)
{
	$total = 0;
	foreach ($arr as $key => $value)
	{
