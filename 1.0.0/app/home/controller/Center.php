...
class Center extends Base
{
	...
	{
		$subject = Subject::getSubject($this->user['school_type']);
		$major_score_key = $subject['subjects'];
		$major_score_arr = [];
		...
		if($this->user['major_score']){
			...
			$score = Db::name('score')->where(['member_list_id' => $this->user['member_list_id']])->find();
			$major_score_arr = json_decode($score['score'],true);
		}
		$total_score = handle_score($major_score_arr);
		$this->assign($this->user);
	...
	private function getMemberInfo()
	{
		...
		$info['watGrade'] = $info['watGrade'] ? json_decode($info['watGrade'],true) : config('watGrade');
		return $info;
	}
