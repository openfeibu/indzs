...
class Score extends Base
{
    public function score_list()
    {
        ...
        $subject = SubjectModel::getSubject('vocational');
        ...
        {
            $map['er.room_name'] = $this->admin['admin_username'];
        }
        if($room_id = input('room_id',0)){
            $map['er.room_id'] = $room_id;
        }
        $score_list = Db::name('examination_member')->alias('em')
                                                    ->join(config('database.prefix').'score s','s.member_list_id = em.member_list_id','left')
                                                    ...
                                                    ->where($map)
                                                    ->field('m.member_list_id,s.score, s.score_id,s.score,s.score_status,m.member_list_nickname , m.member_list_username, m.member_list_id,mi.ZexamineeNumber,em.*,er.*')
                                                    ->order('em.room_id asc')
                                                    ->order('em.id asc')
                                                    ->distinct('m.member_list_id')
                                                    ->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);

        $data = $score_list->all();
        $score_key = $subject['subjects'];
        $status = config("status");
        foreach($data as $key => $val)
		{
            $score_arr = $val['score'] ? json_decode($val['score'],true) : [];
            $score_arr = handle_score_arr($score_key,$score_arr);
            $data[$key]['score_arr'] = $score_arr;
            $score_status = $val['score_status'] ? $val['score_status'] : 0 ;
            $data[$key]['score_status'] = $score_status ;
            $data[$key]['status_desc'] = $status[$score_status] ;
            $score_total = 0;
    ...
    public function score_runadd()
    {
        ...
        $score_data = Db::name('score')->where(array('member_list_id' => $member_list_id))->find();
        if($score_data)
        {
            ...
            $rst = Db::name('score')->where(array('member_list_id' => $member_list_id))->update($data);
			if($rst!==false){
    ...
    public function recruit_score_list_export()
    {
        ...
        $table = '自主'.$recruit_major['recruit_major_name'].'技能考核成绩'.date('YmdHis');
        $title = $recruit_major['recruit_major_name'].'      技能考核成绩';
        $author = '广东农工商职业技术学院      自主';
	...
	public function recruit_score_all()
    {
		...
		foreach($data as $key => $val)
		{
            ...
            $major_score_desc = major_score_desc($major_score_key,$major_score_arr);
            $major_score_arr = handle_major_score_arr($major_score_key,$major_score_arr);
            $data[$key]['major_score_arr'] = $score_arr;
            $data[$key]['major_score_desc'] = $major_score_desc;
            $val_recruit_score_status = $val['recruit_score_status'] ? $val['recruit_score_status'] : 0;
			$data[$key]['status_desc'] = $status[$val_recruit_score_status];
			$data[$key]['major_score_key'] =  $major_score_key;
